...
REPORT = os.path.join(ROOT, "source", "CHANGES.txt")
...
def main():
    ...
    # cleaned variant of every paragraph, kept beside the raw one
    ...
    for e in entries:
        for b in e["blocks"]:
            if b["t"] != "p":
                continue
            fixed = fixlib.apply(b["x"], rules, log)
            if fixed != b["x"]:
                b["c"] = fixed
                n_clean += 1

    with open(REPORT, "w", encoding="utf-8") as fh:
        fh.write("Every change the Cleaned reading mode makes.\n")
        fh.write("Generated by tools/build.py from source/fixes.txt -- do not edit.\n\n")
        by_rule = {}
        for c in log:
            by_rule.setdefault((c["rule"], c["to"]), []).append(c)
        for (rule, to), hits in sorted(by_rule.items(), key=lambda kv: -len(kv[1])):
            fh.write("%s  ->  %s   (%d)\n" % (rule, to, len(hits)))
            for h in hits[:4]:
                fh.write("    ...%s...\n" % h["context"].strip())
            if len(hits) > 4:
    ...
    print("  %d paragraphs cleaned, %d individual fixes -> source/CHANGES.txt"
          % (n_clean, len(log)))
    for c in chapters:
